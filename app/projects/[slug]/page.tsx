...
function imageFrameClass(image: ProjectImage, placement: ImagePlacement): string {
  ...
  return [
    "shine-surface relative overflow-hidden rounded-[1.5rem] border border-[#ff9ac3]/22 shadow-[0_24px_80px_rgba(0,0,0,0.35)]",
    aspectClass,
    widthClass,
    surfaceClass,
  ].join(" ");
  ...
  return (
    <div className="relative min-h-screen overflow-x-clip bg-[#0C0C0C] text-[#D7E2EA]">
      ...
      <main className="relative mx-auto w-full max-w-6xl px-5 pb-20 pt-32 sm:px-6 md:pt-40">
        ...
        <header className="metal-card shine-surface mt-8 overflow-hidden rounded-[2rem] backdrop-blur">
          <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.78fr)] lg:items-center">
            <div>
              <div className="metal-chip inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs uppercase tracking-[0.14em] text-[#F5F7FA]/75">
          ...
          <div className="border-t border-[#ff9ac3]/16 px-6 py-5 sm:px-8">
        ...
        <section className="mt-10 grid gap-5 lg:grid-cols-3">
          <article className="metal-card metal-lift rounded-2xl p-6">
            ...
            <ul className="mt-3 space-y-2 text-sm font-light text-[#D7E2EA]/75">
              {project.impact.map((item) => (
                <li key={item} className="flex items-start gap-3">
                  <span className="mt-2 h-1.5 w-1.5 rounded-full bg-[#d75f92] shadow-[0_0_12px_rgba(215,95,146,0.75)]" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
        ...
        {project.sections?.length ? (
          <section className="mt-5 grid gap-5 md:grid-cols-2">
            {project.sections.map((section) => (
              <article key={section.title} className="metal-card metal-lift rounded-2xl p-6">
